...
public class Poker {
	...
	private boolean checkStraight(HashSet<Card> hand) {
	...
	private HashSet<Card> chckStraigth(HashSet<Card> hand) {
		return null;
	}
...
//		}
		...
		// HashSet<Rank> ranks = makeRanSeet(hand);
...
// 넘어온 hand 내부의 카드 7장으로 어떤 족보인지 맞추고 승패를 가릴 수 있는 점수를 리턴
	...
	public HashSet<Rank> makeRankSet(HashSet<Card> hand) {
		HashSet<Rank> ranks = makeRankSet(hand);
		
		int straith = checkStraight(hand);
		...
		public HashSet<Rank> makeRankSet(HashSet<Card> hand) {
			...
			int straith = checkStraight(ranks);
