...
public class D11_Calender {
	...
	public static void main(String[] args) {
		...
		// * 유닉스타임 - 1970년 1월 1일 이후 시간이 몇 밀리초나 흘렀는지 센 것
		...
		System.out.println("1970년 1월 1일 이후로.. " +
				System.currentTimeMillis() / 100 / 60 + "분 흘렀습니다.");
		...
		/*
		 	...
		 	- 시간을 가져올 때 생성자가 아닌 getInstaance()라는 스태틱 메서드를 사용한다
		 	- getInstaance() 메서드에 경도를 나타내는 TimeZone을 전달할 수 있다
		...
		// Calendar에 저장된 시간 중 필요한 값을 get() 메소드를 통해 꺼낼 수 있다
		int year = korea.get(Calendar.YEAR);
		int month = korea.get(Calendar.MONTH); // Calendar 클래스는 month만
		...
		//Calendar.AM_PM이 필드 상수이고, Calendar.AD과 Calendar.BC 상수이다
		int era = korea.get(Calendar.ERA);
		
		//Calendar.AM_PM이 필드 상수이고, Calendar.AM과 Calendar.PM 상수이다
		...
		int dowin = korea.get(Calendar.DAY_OF_WEEK_IN_MONTH);
        ...
        System.out.println("DOWIM(이번 달의 몇 번째 해당 요일인가): " + dowin);
